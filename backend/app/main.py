...
"""
    ...
    try:
        ...
        return {"ipo_id": ipo_id, "company": ipo.name, "summary": summary_text, "cached": False}
...
def get_risk(ipo_id: str):
    ...
    if de > 1.5:
        factors.append({"factor": "High Debt-to-Equity", "value": ipo.debt_equity, "impact": "negative", "explanation": f"D/E of {ipo.debt_equity} is above the safe threshold of 1.5x. High debt increases financial risk."})
    else:
        factors.append({"factor": "Healthy Debt-to-Equity", "value": ipo.debt_equity, "impact": "positive", "explanation": f"D/E of {ipo.debt_equity} is manageable and within safe limits."})

    if npm < 0:
        factors.append({"factor": "Negative Net Margin", "value": ipo.net_margin, "impact": "negative", "explanation": f"Company is loss-making with net margin of {ipo.net_margin}. Not yet profitable."})
    elif npm < 5:
        factors.append({"factor": "Thin Net Margin", "value": ipo.net_margin, "impact": "negative", "explanation": f"Net margin of {ipo.net_margin} is below 5% — very thin profitability."})
    else:
        factors.append({"factor": "Healthy Net Margin", "value": ipo.net_margin, "impact": "positive", "explanation": f"Net margin of {ipo.net_margin} indicates good profitability."})

    if pe > 50:
        factors.append({"factor": "High Valuation", "value": ipo.pe, "impact": "negative", "explanation": f"P/E of {ipo.pe} is very high. Company is priced aggressively — leaves little room for error."})
    elif pe > 30:
        factors.append({"factor": "Moderate Valuation", "value": ipo.pe, "impact": "neutral", "explanation": f"P/E of {ipo.pe} is moderately high. Growth must justify the premium."})
    else:
        factors.append({"factor": "Attractive Valuation", "value": ipo.pe, "impact": "positive", "explanation": f"P/E of {ipo.pe} is reasonable and attractively valued."})

    if roe > 15:
        factors.append({"factor": "Strong ROE", "value": ipo.roe, "impact": "positive", "explanation": f"ROE of {ipo.roe} shows management is generating strong returns on shareholder equity."})
    else:
        factors.append({"factor": "Weak ROE", "value": ipo.roe, "impact": "negative", "explanation": f"ROE of {ipo.roe} is below 15% — management is not generating enough returns."})
